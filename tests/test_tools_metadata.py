"""list_metadata 테스트 (게이트 2 승인 8케이스 — 단위 C에서 name/n dict 반환으로 갱신)."""
...
def test_brand_list_sorted_by_review_count(conn):
    ...
    assert list_metadata(conn, "brand") == [
        {"name": "브랜드B", "n": 15},
        {"name": "브랜드A", "n": 12},
        {"name": "브랜드C", "n": 5},
    ]
...
def test_category_list(conn):
    # 상의(12+5=17) > 바지(15)
    assert list_metadata(conn, "category") == [
        {"name": "상의", "n": 17},
        {"name": "바지", "n": 15},
    ]
...
def test_sub_category_list(conn):
    # 데님(15) > 티셔츠(12) > 셔츠(5)
    assert list_metadata(conn, "sub_category") == [
        {"name": "데님", "n": 15},
        {"name": "티셔츠", "n": 12},
        {"name": "셔츠", "n": 5},
    ]
...
def test_search_partial_match(conn):
    assert list_metadata(conn, "brand", search="랜드A") == [{"name": "브랜드A", "n": 12}]
...
def test_search_case_insensitive(conn):
    ...
    conn.commit()
    assert list_metadata(conn, "brand", search="engbrand") == [{"name": "EngBrand", "n": 1}]
...
def test_limit(conn):
    assert list_metadata(conn, "brand", limit=2) == [
        {"name": "브랜드B", "n": 15},
        {"name": "브랜드A", "n": 12},
    ]
